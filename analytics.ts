...
export function track(event: string, params: Record<string, unknown> = {}): void {
  ...
  console.log('[IA-TEMP] track → dataLayer.push', { event, ...params }); // TEMP: remove after verifying
...
export function captureLead(params: Record<string, unknown> = {}): string {
  if (typeof window === 'undefined') return '';
  const id = newEventId();
  console.log('[IA-TEMP] captureLead: event_id =', id, params); // TEMP: remove after verifying
  ...
  try {
    ...
    console.log('[IA-TEMP] captureLead: POST /api/capi', payload); // TEMP: remove after verifying
    fetch('/api/capi', {
      method: 'POST',
      ...
      .then(async (r) => {
        ...
        console.log('[IA-TEMP] /api/capi \u2190', r.status, txt); // TEMP: so CAPI can't fail silently
      })
      .catch((err) => {
        console.error('[IA-TEMP] /api/capi request FAILED', err); // TEMP
      });
  } catch (err) {
    console.error('[IA-TEMP] captureLead threw (non-blocking)', err); // TEMP
  }
  return id;
}
