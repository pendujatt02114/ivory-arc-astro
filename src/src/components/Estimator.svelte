<script lang="ts">
  ...
  */
  ...
  function onSend() {
    track('lead_to_ivy', { lang, interest, nights: totalNights, adults, children, destinations: dest.length });
  }
...
<div class="grid gap-8 md:grid-cols-12 md:gap-10">
  ...
  <aside class="md:col-span-5">
    <div class="rounded-xl border border-line bg-cream p-6 md:sticky md:top-24">
      ...
      <a href={ready ? mailLink : undefined} class={`est-cta-sec mt-2.5 w-full ${ready ? '' : 'pointer-events-none opacity-50'}`}>{t.ctaMail}</a>
