...
export interface WarmLead {
  ...
  language?: string;      // English name of the site language the customer chose (e.g. "German")
  adults: number;
  children: number;
  nationality: string;
  vehicle?: 'sedan' | 'suv' | 'tempo' | 'highend';
  explore_delhi?: boolean;     // wants Delhi sightseeing -> Delhi add-ons offered
  delhi_stay?: boolean;        // wants a Delhi hotel -> Delhi becomes an overnight leg
  delhi_nights?: number;
  route_order?: string[];      // ordered city NAMES incl. Delhi book-ends, for the record
  legs: WarmLeadLeg[];         // overnight legs in journey order (Delhi only if delhi_stay)
  leg_km?: number[];
...
export interface EstimatorState {
  ...
  exploreDelhi?: boolean; delhiStay?: boolean; delhiNights?: number;
  routeOrder?: string[];
  legs: WarmLeadLeg[];
  addonsSelected?: WarmLeadAddon[];
}
...
export function buildWarmLead(s: EstimatorState): WarmLead {
  return {
    ...
    currency: s.currency,
    language: s.language && s.language.trim() ? s.language.trim() : undefined,
    adults: s.adults,
    children: s.children,
    nationality: s.nationality,
    vehicle: s.vehicle,
    explore_delhi: s.exploreDelhi,
    delhi_stay: s.delhiStay,
    delhi_nights: s.delhiStay ? s.delhiNights : undefined,
    route_order: s.routeOrder && s.routeOrder.length ? s.routeOrder : undefined,
