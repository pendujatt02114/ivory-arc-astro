<script lang="ts">
  ...
  */
  ...
  const VEH_LABELS: Record<string, string> = { sedan: 'Sedan', suv: 'SUV', tempo: 'Tempo Traveller', highend: 'Mercedes / Audi / BMW' };
  ...
  const selectedAddons = $derived.by(() => {
    const byKey = Object.fromEntries(picked.map((p) => [p.key, p.name]));
    return Object.entries(addonsSel)
      .filter(([, v]) => v)
      .map(([k]) => {
  ...
  async function send() {
    ...
    // The brief travels INSIDE the WhatsApp message, so IVY has the whole trip even
    // if the backend store is unreachable — it adopts this ref and never re-asks.
    waUrl = ref
      ? waBriefHref({
          name, ref,
          legs: picked.map((p) => ({ name: p.name, nights: p.nights })),
          startCity, startDate, adults, children, nationality,
          themes: themes.map((t) => themeLabel(t)),
          vehicleLabel: vehicle ? VEH_LABELS[vehicle] : undefined,
    ...
    <div class="card">
      ...
      {#if step === 7}
        ...
        <ul class="review">
          ...
          <li>{L.vehQ}: {vehicle ? VEH_LABELS[vehicle] : L.vehRecommend}</li>
          <li>{L.steps[6]}: {selectedAddons.length ? selectedAddons.map((a) => a.name).join(' · ') : '—'}</li>
        </ul>
